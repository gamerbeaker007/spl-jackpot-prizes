...
import { RecentWinner, SplCardDetail } from "@/app/types/shared";
...
export function RecentWinnersCarousel({ winners, cardDetails }: Props) {
  ...
  const maxOffset = Math.max(0, winners.length - VISIBLE_CARDS);
  ...
  return (
    <Box sx={{ display: "flex", alignItems: "center", gap: 1, width: "100%" }}>
      <IconButton onClick={handlePrev} disabled={offset === 0} sx={{ flexShrink: 0 }} aria-label="Previous winners">
        <ArrowBackIosNewIcon />
      </IconButton>

      {/* Viewport */}
      <Box
        sx={{ flex: 1, overflow: "hidden", cursor: isDragging ? "grabbing" : "grab" }}
      ...
      <IconButton onClick={handleNext} disabled={offset >= maxOffset} sx={{ flexShrink: 0 }} aria-label="Next winners">
        <ArrowForwardIosIcon />
      </IconButton>
    </Box>
  );
