...
import { getPeakMonsterPricesAction, getPeakMonsterTopBidsAction } from "@/lib/actions/peakMonsterPrices";
import { useCallback, useEffect, useState } from "react";
import { PKBid, PKPrice } from "../types/pkPrices";
...
export function usePeakMonsterPrices(): UsePeakMonsterPricesReturn {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const [pricesData, bidsData] = await Promise.all([getPeakMonsterPricesAction(), getPeakMonsterTopBidsAction()]);
        if (!cancelled) {
